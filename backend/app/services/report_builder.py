...
def csv_cell(v: Any) -> Any:
    ...
    template spelling, so exports round-trip through the importer); everything else unchanged."""
...
def quality_pdf(username: str, q: dict[str, Any]) -> bytes:
    story: list[Any] = _header_block("Analytics quality (ANPR accuracy evidence)", [f"Window (IST): {q['window']['from_ist']} → {q['window']['to_ist']}", f"Generated by {username}"])
    rows = [
