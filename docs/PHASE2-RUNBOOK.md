...
Rehearsal timings: `[FILL: (a) mock ____ s, (b) sandbox ____ s, (c) CSV ____ s]`.
...
| Evidence | Route PDF footer hash; alert drawer → **Play recording**; **Create clip** → SHA-256; `/reports` History → **Verify** | Chain of custody; every export audited |
...
| End-to-end performance | Alert `latency_ms` on the panel; dashboard avg latency; worker fps on `/health` | "Read → alert on screen p95 `[MEASURE]` ms" |
