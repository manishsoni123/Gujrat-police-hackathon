...
| `mediamtx` | `bluenviron/mediamtx:1.20.1-ffmpeg@sha256:16c56911dbd1fd7a85455042fa8320779fc8bf6575c6903f74023b202c1a620c` (tag and digest pinned in `deploy/docker-compose.yml`; recorded in `docs/LICENCES.md`) | 8554 RTSP, 8889 WHEP/WebRTC, 8888 HLS, 9996 playback, 9997 API, 8189 ICE mux | **8189/udp + 8189/tcp** (WebRTC ICE); 8554/tcp for debugging only (commented out in prod) | always | Relay. Config `deploy/mediamtx.yml` mounted at `/mediamtx.yml`; volumes `recordings:/recordings`, `./media:/media:ro` |
...
| `/wall` | Video wall | all | Grid 4 / 9 / 16; 4 and 9 = live WHEP tiles; 16 = 4 live + 12 snapshot tiles (`snapshot_url` refreshed every 1 s via WS `snapshot` or polling `?t=`); camera picker per slot; layout saved with `PUT /me/wall-layout`; "2 systems" badge shows sandbox vs own feeds |
...
| `/alerts` | Alerts | all | Panel sorted by priority then time; filters status/type/priority; escalation badge; Ack/Close with note (`alerts.ack`); detail drawer with crop, camera, map mini, "Play recording", "Export clip"; `?id=` opens one |
...
9. `/alerts` → **Play recording** on the alert → 30 s clip from 10 s before the read; **Export clip** → hash shown; `/reports` History → **Verify**.
...
| 2026-09-05 | docs | §1.1 / §11.2 / §13 | Wording aligned with the built UI: the wall badge reads **"2 systems"** (`WallPage.tsx`) and the recording action is **"Export clip"** (`Recordings.tsx`); the MediaMTX image row carries the compose pin `1.20.1-ffmpeg@sha256:16c56911…`. `GET /departments` is documented in `REGISTRY-API.md` §3.10. Deliverable tooling lives in `docs/tools/` (`export-pdf.mjs`, `build-presentation.mjs`, `moment-shots.mjs`). | Docs must match screen text |
| 2026-09-05 | backend | §4.1 / §5.6 / §5.5 / §5.14 | New camera status **`not_streaming`**: a camera that has **never** delivered a stream (`last_seen_at IS NULL` — catalogue `live=false`, or a source that never answered) reaches this state after `HEALTH_OFFLINE_AFTER` failed checks instead of `offline`; it raises **no** `camera_offline` alert, event, toast or webhook (only the `camera_health_log` row, the status change and the `/ws/health` `health` envelope). `offline` — and the alert/event/webhook of §5.6 rule 5 — is reserved for a camera that was `online`/`degraded` before. `not_streaming` → ready is `online` without a `camera_online` event. Counts (`cameras.not_streaming`) are additive in `/health/summary`, `/dashboard/stats`, `stats` envelopes and gap-analysis per-area rows; `/health/summary.not_streaming[]` lists them and `down_over_5min` never includes them. `uptime_24h_pct` ignores `source_flag='catalogue'` health rows (monitored cameras only). Startup migrates existing offline-never-seen rows once and closes their auto-raised alerts (`outcome='other'`, note `catalogue reports not live`, audit `alert.auto_close`). Pure state machine: `services/health_poller.decide_status`, `tests/test_health_state.py`. | 42 permanent, escalated camera_offline alerts two minutes after the first catalogue import buried the demo's watchlist hits; the acceptance rows describe 9 real transitions |
| 2026-09-05 | backend | §7.5 / §5.5 / §9 / §4.2 | Heartbeat `extra` (object; ≤ 16 KB) is persisted in `anpr_workers.extra` (JSONB) and every worker item (`/health/summary`, `/dashboard/stats`, `anpr_status`) carries `extra`, `started_at`, `version`, `detector`, `camera_states` and a derived `degraded` (`extra.detector.degraded` true, or requested ≠ active with requested ≠ `auto`). `mode` must match `^(live\|preindex)$` (422). `camera_states`/`cameras`/`fps_total` are scoped per caller/connection (a dept_admin only sees its own cameras; `anpr_status` keeps `worker_id` and the `cameras` list, adds `camera_count`). The retention job deletes `anpr_workers` rows whose `last_heartbeat_at` is older than 24 h (`after.anpr_workers`). | Detector fallback state was accepted and dropped; unscoped broadcast; stale rows |
| 2026-09-05 | backend | §1.3 / §5.22 / §2.6 / §2.7 | On a **public** deployment (`COOKIE_SECURE=1` or `https` `PUBLIC_BASE_URL`) seeded API keys / jury passwords still equal to the repository defaults do **not** stop the API (JWT_SECRET / POSTGRES_PASSWORD remain fatal) but: a loud `SECURITY WARNING` banner is logged at startup, `GET /healthz` returns `default_secrets_in_use: true` (boolean, no names; `/health/summary` adds the env names as `default_secrets[]` for admins) so the SPA shows an admin banner, and the seeded **bulk** key equal to `BULK_API_KEY`'s default is created/kept **inactive** (`seed.api_keys.deactivated`) because `/api/v1/cameras/bulk` is Internet-reachable through Caddy; the internal key stays active (Caddy blocks `/api/internal/*`). On the plain-HTTP laptop the field is always `false`. | Documented default credentials on the hosted demo |
| 2026-09-05 | backend | §7.2 / §7.6 / §4.2 | Worker events are replay-safe like reads: partial unique index `uq_events_worker_dedup (camera_id, type, occurred_at, coalesce(note,''))` `WHERE is_auto AND type IN ('loop_reset','intrusion')`, insert `ON CONFLICT DO NOTHING`; a replayed event is reported in `rejected` as `{"kind":"event","reason":"duplicate"}`, writes no frame file, does not bump the intrusion alert's `read_count` and is not broadcast. Existing duplicates are collapsed once at startup (lowest id kept). | Batch retries after a timeout doubled events, `read_count` and `loop_resets_in_window` |
| 2026-09-05 | backend | §5.10 | `GET /vehicles/search`: `exact` is ordered `first_seen` **desc** (the route keeps asc); sightings whose `plate_norm` equals the query are never fuzzy candidates, even beyond the `limit` cap; `cameras_seen` counts the **uncapped** exact set (+ confirmed fuzzy); response adds `exact_total`, `fuzzy_total` and `limit` so the UI can say "50 of 1114". | Oldest-first paging hid recent sightings and leaked exact rows into `fuzzy` |
| 2026-09-05 | backend | §5.12 rule 3 / §4.2 / §1.3 | Re-alert window is the setting **`alerts.re_alert_minutes`** (env `ALERT_RE_ALERT_MINUTES`, default **60**, `0` = an open alert absorbs reads indefinitely; range 0–1440) — not the fixed 10 min of rule 3. Add it to §4.2's seeded list after `alerts.suppression_s` and to the §1.3 env table. `MatchEngine.should_suppress(created_at, status, now, suppression_s, re_alert_minutes)`. | The demo plate loops every 90 s; 10 min produced a fresh critical alert per camera six times an hour |
| 2026-09-05 | backend | §3.2 | `page_size` above the endpoint's maximum (200; 500 for `/detections`, `/detections/sightings`, `/audit`) is `422 {"errors":[{"field":"page_size","message":"must be <= 200"}]}` naming that maximum — never silently clamped. 500 is the absolute ceiling for every list. | Clients paging with 300 received 200 rows unnoticed |
| 2026-09-05 | backend | §4.4 | Additional audit actions: `settings.catalogue_test` (`POST /settings/catalogue/test`), `webhook.test`, `webhook.update`, `user.wall_layout`; `auth.login_failed.after.reason` ∈ `rate_limited`, `rate_limited_user`. The middleware never writes a row for an **unmatched** route (404) and skips `/api/internal`, `/api/mock-sandbox`, `/api/auth/verify`, `/api/healthz` with or without a trailing segment (no synthetic `post.api.internal` rows). | Exhaustive list was incomplete; synthetic rows |
| 2026-09-05 | backend | §2.1 | Login rate limiting: per-IP window (`LOGIN_RATE_LIMIT_PER_MIN`, every attempt) is **never** cleared by a successful login; a second limiter per lower-cased username counts **failed** attempts only — 5 failures in 15 minutes lock that username (`429`, audit `auth.login_failed` reason `rate_limited_user`); a successful login resets only that username's counter. Single-process, in memory. | A known viewer login unlocked unlimited guesses at `jury_admin` |
| 2026-09-05 | backend | §2.2 / §5.20 / §4.2 | Token revocation: `users.token_not_before` (timestamptz) is set to now by `POST /users/{id}/reset-password`, `POST /auth/change-password`, `DELETE /users/{id}` and any `PUT /users/{id}` that changes `role`, `department_id`, `district` or deactivates; a JWT whose `iat` (integer seconds) is below `ceil(token_not_before)` is rejected (`401`) by REST, WebSocket and `/auth/verify`. `change-password` therefore also ends the caller's own session (cookie cleared; the SPA must return to the login page and sign in with the new password). Tokens require `exp`, `iat`, `sub`; library is **PyJWT 2.9** (MIT) instead of python-jose. | Sessions survived a password reset for up to 8 h |
| 2026-09-05 | backend | §2.1 | `?token=<jwt>` is honoured **only** on `/ws/*` and `/media/*` (and by `/auth/verify` through the forwarded URI); on `/api/*` REST routes it is ignored (`401`), so a pasted link never puts a session token into Caddy's access log. | Contract said WebSocket/download links only |
| 2026-09-05 | backend | §5.18 | `GET /evidence/verify` answers **`404 Not found`** (audit `evidence.verify` with `found=false`) for a path that is not an in-scope ledger row — the same answer `/media` gives — instead of hashing an arbitrary file under `DATA_DIR`. Admins may pass `?any=1` to verify an unledgered file (`entity=null`). | Existence/size/SHA-256 oracle across departments |
| 2026-09-05 | backend | §5.20 / §1.3 | Outbound-URL guard (`core/urlguard.py`) for `catalogue.base_url`, `POST /settings/catalogue/test.base_url` and webhook URLs: loopback, link-local (`169.254/16`), RFC 1918 / ULA, multicast, reserved, single-label and `.local/.internal` hosts — after DNS resolution — are rejected (`422`) unless `ALLOW_PRIVATE_URLS=1` **or** `MOCK_SANDBOX=1` (the laptop's `http://api:8000/…` targets). The check also runs at use time (catalogue fetch → `CatalogueError`, webhook delivery → `last_error = "rejected: …"`). The catalogue fetch no longer follows redirects (a 3xx is an error naming the `Location`; set `base_url` to the final URL); `catalogue/test.sample` is truncated above 8 KB (`_truncated: true`). New env `ALLOW_PRIVATE_URLS` (default `0`). | SSRF from the API container |
| 2026-09-05 | backend | §5.22 | `POST /mock-sandbox/webhook-sink` stays open **only at the API root** (`http://api:8000/mock-sandbox/webhook-sink`, compose network, unreachable through Caddy); `POST /api/mock-sandbox/webhook-sink` requires `X-API-Key` with scope `internal`. `GET /api/mock-sandbox/webhook-sink` (admin) and `GET /api/mock-sandbox/api/ingest` are unchanged. The check-25 test webhook URL is therefore `http://api:8000/mock-sandbox/webhook-sink`. | Unauthenticated write to an admin-visible list |
| 2026-09-05 | backend | §10.3 / §5.15 / §5.2 | Every CSV the API writes (exports, reports, templates, import error reports) neutralises spreadsheet formulas: a string cell starting with `=`, `+`, `-`, `@`, TAB or CR is prefixed with `'` and C0 control characters are stripped (`csv_importer.neutralise_cell`, applied by `report_builder.csv_cell`); the `# sha256(rows)=` trailer is computed on the escaped spelling that is written. Numbers/booleans are unchanged. Retention `after` gains `anpr_workers`. | CSV/formula injection via camera names, User-Agent, uploaded lines |
| 2026-09-05 | anpr | §7.2 / §7.7 | Voted reads now carry a `plate_raw` that **agrees with the vote**: `anpr/voting.py` (`raw_for_vote`) takes the raw OCR string of the highest-confidence bucket member whose own normalisation equals the voted `plate_norm` (fallback `format_plate(plate_norm)`), while bbox/crop/frame/time still come from the best member; invariant `normalise(read.plate_raw).plate_norm == read.plate_norm` is asserted in `anpr/tests/test_voting.py`. Backend behaviour (re-normalise `plate_raw`, override `plate_norm`) is unchanged. | The API's re-normalisation of `plate_raw` silently undid every voting correction (live DB: read 4193 raw `GJ 01 AB 1284` stored as `GJ01AB1284` under sighting `GJ01AB1234`), defeating the §7.7 design |
| 2026-09-05 | anpr | §12.4 / §12.3 / §8.2 | `scripts/make_synthetic_videos.py` also writes `media/synthetic/own_gate.mp4` (H.264, 90 s; dusk green-grey scene with gate pillars and a striped boom) plus a `plates.json` → `own_gate` section (`file`, `eval_camera_id` 99, `shared_with_streams`, `plates`, `appearances`). Its plate set is disjoint from the eight stream loops except **exactly two** shared plates that also pass cameras 1–3: `GJ01AB1234` at 10 s and `GJ27XY3456` at 28 s (the script raises otherwise). `deploy/mediamtx.yml` `own_gate` should fall back to `/media/synthetic/own_gate.mp4` (not `cam_1.mp4`) when `/media/own/own_gate.mp4` is absent; `anpr.tools.eval_synthetic --own-gate` scores it. `--only 99` re-renders it alone; `--no-own-gate` skips it. | The own feed replayed camera 1's loop, so the "two systems" route demo showed identical scenes and identical plate sets |
| 2026-09-05 | anpr | §12.4 | Burnt-in overlay of every synthetic loop is now one small grey OSD strip at the **bottom-right**: `cam 1 · sachivalaya gate 1 · loop 00:12.3` (own feed: `own gate · dynatech office gate · loop …`). The top-left yellow label and the top-right wall clock are gone; `plates.json` carries an `osd` note. | The label sat under the player's title chip and the generation-time clock contradicted the portal's IST header on every tile (review finding) |
| 2026-09-05 | anpr | §1.1 | `anpr/Dockerfile` runs the worker as the unprivileged user `anpr` (uid/gid 10001, `HOME=/app`, PaddleOCR models in `/app/.paddleocr`, `USER anpr` before `CMD`); the container writes nothing at run time and was verified with `--read-only --tmpfs /tmp --security-opt no-new-privileges:true`. Deploy may add `security_opt: [no-new-privileges:true]`, `read_only: true` and `tmpfs: [/tmp]` to the four `anpr-*` services. | Worker decoded untrusted RTSP bitstreams as root while holding `INTERNAL_API_KEY`; the api image already dropped privileges |
| 2026-09-05 | deploy | §1.1 / §1.3 | `deploy/docker-compose.yml` publishes the MediaMTX control API (9997) and playback server (9996) on `127.0.0.1` unconditionally; `MTX_BIND` (and `docker-compose.debug.yml`) only widen 8554/8888/8889. `authInternalUsers` grants `any` the `api` action without a password, so `MTX_BIND=0.0.0.0` must never expose path add/delete (`runOn*` = shell commands in the relay container) or recordings to a LAN. | Security review |
| 2026-09-05 | deploy | §1.3 | `deploy.sh` generates `POSTGRES_PASSWORD` (32 hex) together with `JWT_SECRET` / API keys on a fresh `deploy/.env`, applies it with `ALTER USER` when the `sentinel_pgdata` volume already exists, and on a public deployment (`--domain`, or `COOKIE_SECURE=1` / https `PUBLIC_BASE_URL`) exits before the build while `POSTGRES_PASSWORD`, `JWT_SECRET` (default or < 32 chars) or an API key is still the repository default (the API guard refuses to boot with them). `--domain` sets `MOCK_SANDBOX=0` unless `--mock`; `--sandbox-url/--sandbox-auth/--sandbox-user/--sandbox-password/--sandbox-header` seed `SANDBOX_*`; the public IP is detected whenever `--public-ip` is absent; the nightly backup crontab is installed with `--domain` (`--no-cron`). New `deploy/rotate-db-password.sh` (`ALTER USER` + `.env` + api restart; value must be `[A-Za-z0-9_.-]`). `backup.sh --install-cron` runs at `RETENTION_JOB_HOUR_UTC + 1` (22:00 UTC / 03:30 IST), `umask 077`, optional `BACKUP_PASSPHRASE` AES-256 encryption. Caddy's JSON access log redacts `?token=` (`format filter … request>uri query replace token REDACTED`). | Ops/correctness review: fresh-VM `--domain` path could not start; password rotation undocumented; JWTs in access logs |
| 2026-09-05 | integrator (release) | §13.3 checks 10, 11, 25 / §8.2 / §1.1 / §1.3 | Final acceptance run on the working tree of 5 Sept (evening): check 10 now passes with `cameras.online ≥ 9`, **`not_streaming == 42`, `offline == 0`** and `down_over_5min == 0` (the 42 catalogue cameras with `live=false` never streamed, so per the `not_streaming` amendment they raise no `camera_offline` alert — `scripts/integration_checks.mjs a` asserts that no `camera_offline` alert, open or closed, exists for a camera with `last_seen_at IS NULL`); check 11's `offline` + alert path is unchanged for a camera that was online before. Check 25 uses the root sink `http://api:8000/mock-sandbox/webhook-sink`. The rate-limit probe uses a throw-away username (five failures lock a username for 15 min). `deploy/mediamtx.yml` `own_gate` prefers `/media/own/own_gate.mp4`, then the generator's `/media/synthetic/own_gate.mp4`, then `cam_1.mp4`. `ALERT_RE_ALERT_MINUTES` is passed through compose and listed in `.env.example`. The CPU `anpr-*` services run with `no-new-privileges`, `read_only` and a `/tmp` tmpfs. `make deliverables` builds `docs/export/`. §11: `StreamPlayer` ends the WHEP session (`DELETE` on the session URL, `keepalive`) and closes the `RTCPeerConnection` only once the DELETE has been answered (capped at 1.5 s) — `pc.close()` sends DTLS close_notify at once and MediaMTX drops the session within milliseconds, so closing first (or merely dispatching the DELETE first) answered 404 and logged a console error on every tile switch/unmount; `frontend/e2e/final_journey.mjs` (31 captures of the §13.2 journey, `docs/screenshots/final/`) requires zero console errors. | Release-manager acceptance run |
| 2026-09-05 | backend | §5.2 / §6 / §1.3 / §4.2 | **Organiser sandbox as a first-class source.** New adapter `adapters/sentinel_portal.py` (`CameraSourceAdapter`, `source='sandbox'`) selected by the new setting **`catalogue.source`** ∈ `mock` \| `sentinel_portal` \| `generic_json` (env `CATALOGUE_SOURCE`; empty = `mock` while `MOCK_SANDBOX=1`, else `generic_json`). Catalogue modes: **upload** (`POST /cameras/import/sandbox/file`, multipart `cameras_json` + optional `enrichment_csv` + `dry_run`/`measure_first_stream`/`probe`; both files are kept under `DATA_DIR/catalogue/` as the "last upload"), **portal** (login to `catalogue.portal_url` `/auth/login` with `catalogue.portal_email`/`portal_password` — the form fields are discovered from the login page, cookie jar via httpx — then `GET /cameras.json`), **file** (last upload, else `catalogue.cameras_json_path`, default `/app/media/cameras.json`). Without a portal password the importer falls back to the file and says so in `warnings`. `cameras.json` is an array of `{id, name}` only: `id` → `external_id` **verbatim** (MediaMTX path ids are case-sensitive), `name` → `name` verbatim (numeric prefix kept); department / lat / lon / district / police_station / type / confidence come from the enrichment CSV (`catalogue.enrichment_path`, default `/app/media/cameras_enrichment.csv`, or `POST /settings/catalogue/enrichment` upload) keyed by `external_id`; no row → no coordinates, department `UNASSIGNED`, warning. `POST /cameras/import/sandbox` (JSON body, now with `probe`) uses this adapter when `catalogue.source = sentinel_portal`, else the §6 `/api/ingest` importer unchanged. Response adds `source`, `catalogue_mode`, `enrichment_source`, `enrichment_rows`, `enrichment_confidence`, `missing_enrichment`, `probed`, `probe_ok`, `probe_h265`, `probe_ms`, `probes[]` (`external_id, name, ok, codec, resolution, fps, live, duration_ms, error, location_confidence, department_code, transcode`). Audit `camera.import_sandbox` stores counts only. | Real sandbox verified 5 Sept: catalogue behind a web session, no metadata |
| 2026-09-05 | backend | §4.2 / §5 `CameraImportRow` / §5.3 | `cameras.location_confidence` (`exact` \| `approx` \| `guess`, CHECK `ck_cameras_loc_conf`) and `cameras.metadata` (JSONB; ORM attribute `meta`) are added by `POST_CREATE_SQL` (`ADD COLUMN IF NOT EXISTS`, so an existing volume migrates on start). `CameraImportRow`/`CameraUpdate` accept `location_confidence` and `metadata` (object ≤ 16 KB; updates shallow-merge top-level keys, `null` deletes a key). `Camera` responses carry both; `GET /geo/cameras` properties carry `location_confidence`. The sandbox importer writes `metadata.enrichment` (`confidence, source, notes, city, type_guess, department_guess, name_matches_catalogue`), `metadata.probe` (import-time ffprobe) and `metadata.catalogue` (`source, mode`). | Team-inferred coordinates must be flagged as such on the map and in the drawer |
| 2026-09-05 | backend | §4.2 `rtsp_url` / §5.2 / §5.4 / §4.4 | **Credentials in URLs are masked for every role and every URL field** (`rtsp_url`, `whep_url`, `hls_url`) in list/detail/create/update responses, the cameras CSV export, audit `before`/`after` diffs, import warnings and probe errors: `serializers.mask_url` (`scheme://user:***@host/…`, any scheme, passwords containing `@`/`:`), `mask_secrets_in_text` for free text (plain, `%`-encoded and fully-encoded spellings) and `mediamtx_client._scrub` for MediaMTX error bodies. `mask_rtsp(url, is_admin)` keeps its signature but ignores `is_admin`. The unmasked URL is used only for MediaMTX path creation (§8.3); the ANPR worker only ever receives relay URLs. An edit form echoing the masked value back (`PUT /cameras/{id}` with `rtsp://user:***@…`) leaves the stored URL unchanged (`is_masked_form`). Regression tests in `tests/test_sentinel_portal.py`. | `GET /cameras` returned the sandbox access password to admins for the CSV-imported rows |
| 2026-09-05 | backend | §5.2 import rules | The organiser-sandbox importer looks existing rows up by `external_id` **across every source** (`camera_importer.pick_existing`: same source first, then non-retired, then lowest id) and converts a match to `source='sandbox'` **in place** (same `id`, relay path `cam_<id>`, ANPR/recording flags), reported as `updated` with a `source` warning; it never creates a duplicate. The `(source, external_id)` uniqueness and the per-source lookup of the CSV / bulk / manual paths are unchanged. | 30 real cameras were first loaded through the CSV path (`source='csv'`) on 5 Sept 13:57 IST |
| 2026-09-05 | backend | §5.2 / §5.6 probes | Import-time probe: `ffprobe -rtsp_transport tcp` with a `sandbox.probe_timeout_s` (30 s since the 5 Sept health-poller amendment; 12 s before) cap, at most `sandbox.probe_parallel` (≤ 6) in parallel, directly against the sandbox URL (one short connection per camera, closed when done). Result → `codec`, `resolution`, `fps` (from `avg_frame_rate`/`r_frame_rate` only when 1–60, else null — cam06 reports 90000/1; timing is driven from PTS anyway), `live` (true on success, false when the server refused the path (4xx/5xx/"path not configured"), **null** on timeout/network error so the health poller keeps checking); H.265 cameras get the `_h264` transcode path as before. 30 cameras complete in ≤ ~90 s. Health poller budget ("pace your load"): at most `HEALTH_PROBE_PARALLEL` (env, default 6, hard cap 6) concurrent probes, at most `HEALTH_PROBE_MAX` per tick, only cameras whose catalogue `live` is not `false`, and an idle camera is probed at most once per `HEALTH_PROBE_MIN_INTERVAL_S` (default 300 s; its last result stands in between; `health_poller.select_probe_candidates`, least-recently-probed first). | Organiser rules: each client gets its own stream copy; pace the load; never trust reported fps |
| 2026-09-05 | backend | §5.20 / §5.22 / §8.3 | Settings: `catalogue.source` (public), `catalogue.portal_url` / `portal_email` / `portal_password` (secret) / `enrichment_path` / `cameras_json_path`, `sandbox.stream_host` (bare host, default `103.250.160.189`) / `rtsp_port` 8554 / `whep_port` 8889 / `hls_base` / `stream_email` / `stream_password` (secret) / `probe_timeout_s` / `probe_parallel`, seeded from env `SANDBOX_STREAM_EMAIL`, `SANDBOX_STREAM_PASSWORD`, `SANDBOX_PORTAL_*`, `CATALOGUE_*`, `SANDBOX_PROBE_*`; secrets follow the `********` = unchanged rule. Stream URLs are built as `rtsp://{quote(email)}:{quote(password)}@{host}:{rtsp_port}/stream/{id}` (the `@` of the e-mail is `%40`; ffmpeg and MediaMTX decode userinfo), `whep_url = http://…:{whep_port}/stream/{id}/whep` and `hls_url = {hls_base}/{id}/index.m3u8` are stored for reference only. The §8.3 relay body for these cameras carries the credentialled URL as `source`, `rtspTransport: tcp`, `sourceOnDemand: true`. `POST /settings/catalogue/test` with `source: sentinel_portal` (or when it is the saved source) probes one camera (`camera_id`, default `cam01`) with the stored/overridden credentials and reports `probe.summary` (`"cam01: H.264 1920x1080"`), the catalogue path (portal or file, count) and the enrichment path — never a secret. `GET /settings/public` adds `catalogue_source`, `catalogue_source_label`, `sandbox_stream_host` (sentinel only) and `mock_sandbox_served`; **`mock_sandbox` (the MOCK SANDBOX badge) is true only while `MOCK_SANDBOX=1` and `catalogue.source = mock`**, so selecting the real sandbox removes the badge without a redeploy. | R12: sandbox host + credentials editable in Settings; badge must reflect the real source |
| 2026-09-05 | deploy | §1.1 / §1.3 | `api` mounts `./media` a second time at `/app/media:ro` (the organiser catalogue copy `media/cameras.json`, the team enrichment CSV and `sandbox_survey.json`); compose passes `CATALOGUE_SOURCE`, `SANDBOX_STREAM_HOST/RTSP_PORT/WHEP_PORT/HLS_BASE/STREAM_EMAIL/STREAM_PASSWORD`, `SANDBOX_PORTAL_URL/EMAIL/PASSWORD`, `CATALOGUE_ENRICHMENT_PATH`, `CATALOGUE_CAMERAS_JSON_PATH`, `SANDBOX_PROBE_TIMEOUT_S`, `SANDBOX_PROBE_PARALLEL`, `HEALTH_PROBE_PARALLEL`, `HEALTH_PROBE_MIN_INTERVAL_S` (all in `.env.example`). `SANDBOX_STREAM_PASSWORD` is the only place the access password is written on disk besides the settings table; it must never be copied into the repository, docs, screenshots or logs. | Real sandbox wiring |
| 2026-09-05 | backend | §5.6 / §8.3 / §1.3 | **Health poller on the real sandbox.** An idle camera whose source is *external* (`mediamtx_client.is_external_source`: the URL is not on our own MediaMTX) is probed **directly** against its source URL (`health_poller.probe_target`) with the `sandbox.probe_timeout_s` cap — default now **30 s** (env `SANDBOX_PROBE_TIMEOUT_S=30`, was 12; the sandbox answers a DESCRIBE in 4–38 s) — instead of through `cam_<id>`: the relay probe with the 6 s `HEALTH_PROBE_TIMEOUT_S` expired before the sandbox answered and then left a 60 s on-demand copy of the stream running, so 7–10 real cameras flapped online/offline every tick. Local sources keep the relay probe with the short cap. Budget unchanged: ≤ `HEALTH_PROBE_PARALLEL` (≤ 6) at once, ≤ `HEALTH_PROBE_MAX` per tick, never a camera with catalogue `live=false`, an idle camera at most once per `HEALTH_PROBE_MIN_INTERVAL_S` (300 s). The §8.3 relay body for an external source carries `sourceOnDemandStartTimeout = max(15, probe_timeout + 10) s` (40 s; `on_demand_start_timeout_s`), local sources keep 15 s, so a slow sandbox camera can come up for the wall/ANPR; the value is applied by the importer's path PATCH, no MediaMTX restart. Verified 5 Sept 16:00 IST: 30 fetched / 0 added / 30 updated (csv → sandbox in place, ids 61–90 and `cam_61..cam_90` kept, 6 `_h264` paths), probe_ok 29/30 (cam30 timed out at 34 s → `live=null`, kept under watch), first stream 5.0 s, 92 s total. `tests/live/phase4_sandbox_secrets.py` scans every URL-rendering endpoint for the access password. | 10 real cameras reported offline although every one answered ffprobe |
| 2026-09-05 | frontend | §11.2 / §11.3 | Settings → Catalogue: source selector (`catalogue.source`), organiser stream host/ports/e-mail/access password (masked, "unchanged" semantics), optional portal login, server-side paths, enrichment CSV upload, **Test** showing `cam01: H.264 1920x1080` + catalogue/enrichment status; the mock/generic host + field-map cards stay for the other sources. Import page: **Import from Sentinel sandbox** card (`cameras.json` upload when no portal login, optional enrichment CSV, probe/measure/dry-run switches, per-camera probe table with codec/resolution/fps/live/location confidence/department, enrichment confidence counts) replaces the generic card while the source is `sentinel_portal`. Map markers: `location_confidence` exact = solid, approx = dashed ring, guess = hollow ring, marker title and popup line "Approximate location (team-inferred)" / "Location is a guess (team-inferred)", legend rows when such cameras exist. Camera drawer and detail page show a **Location confidence** row with the enrichment source and note. About page names the real camera source (`catalogue_source_label`). No page ever renders a raw credential (URLs arrive masked). | Real sandbox UI |
| 2026-09-05 | integrator (sandbox proof) | §11.3 / §5.6 | `StreamPlayer`: the WHEP offer has its own budget — **45 s** when `GET /streams/{id}` reports `ready: false` (the relay holds the answer while it pulls an idle on-demand source; the organiser sandbox answers a DESCRIBE in 4–38 s and external paths carry `sourceOnDemandStartTimeout` 40 s), 10 s when the path is already ready — and the 5 s ICE budget starts only after the answer; the hls.js manifest budget follows the same rule. A playing WebRTC session that loses its transport (relay source gap/reconnect) restarts the WebRTC → HLS → snapshot chain instead of dropping to HLS. `sandbox.probe_timeout_s` is set to 45 on the laptop (cam15/cam30 answer in 18–38 s). Verified against the real sandbox: with the fixed 5 s budget every cold tile fell to snapshot mode before the source came up. | Real-sandbox wall verification |
| 2026-09-05 | integrator (sandbox proof) | §8.3 / §5.4 / §5.2 / §11.3 | **B-frame H.264 sources get the `_h264` re-encode like H.265.** MediaMTX refuses a WebRTC reader for them ("WebRTC doesn't support H264 streams with B-frames") and its HLS muxer dies on long reorder chains ("unable to extract DTS: too many reordered frames (11)") — sandbox cam09/cam24/cam28/cam29 (`has_b_frames` 2). The probe records ffprobe's `has_b_frames` as `metadata.probe.b_frames` (also `b_frames` / `transcode` in the import report row); `services/mediamtx_client.needs_transcode(codec, meta)` (H.265 or `b_frames > 0`) drives `play_path(camera_id, codec, meta)`, `ensure_camera_paths` (creates `cam_<id>_h264`, records on it) and every play-path consumer (`/streams`, `/clips`, `/recordings`, `Camera.play_path`). `GET /streams/{id}` adds `transcoded` (bool), `whep_supported` (bool) and `preferred_mode` (`whep` | `hls`); the player starts on HLS when `whep_supported` is false (a camera probed before its path set was refreshed) and after two WebRTC sessions in a row that die within 10 s of connecting; the caption reads "H.264 (re-encoded)". Cost: one libx264 process per such camera while it is watched (on demand, closed after 60 s idle). | 4 of 9 real wall tiles stayed black over WebRTC and HLS |
| 2026-09-05 | integrator (sandbox proof) | §8.3 | CPU transcode command (`MEDIAMTX_TRANSCODE=cpu`) is `ffmpeg -nostdin -loglevel error -rtsp_transport tcp -i rtsp://localhost:8554/cam_<id> -vf "scale=-2:'min(720,ih)'" -c:v libx264 -preset ultrafast -tune zerolatency -b:v 1200k -g 30 -an -f rtsp rtsp://localhost:8554/cam_<id>_h264` (720p cap, never upscales, cheapest preset). NVENC command unchanged. | 17 of the 30 sandbox cameras need the re-encode (6 H.265 + 11 B-frame H.264); five 1080p `veryfast` encoders drove MediaMTX to 465 % CPU / 970 MB under the 4-core cap and the on-demand starts timed out (`runOnDemand command stopped: timed out`) |
| 2026-09-05 | anpr (sandbox survey) | §1.3 / §7.7 | **Tiled plate detection.** New worker variables `ANPR_DET_TILE` (px, default `0` = off) and `ANPR_DET_TILE_OVERLAP` (default `0.15`): when set, `anpr.detector.TiledDetector` runs the configured detector on the whole frame *and* on overlapping tiles of that size (1920 px frame, 768 px tiles → 6 tiles + whole frame ≈ 7 inferences ≈ 60 ms on the laptop CPU), drops tile boxes that cannot be a plate (wider/taller than half the tile, aspect outside 0.12–0.9) and merges duplicates by IoU ≥ 0.4. Heartbeat `extra.detector.tile` carries the size. Needed because the organiser sandbox cameras are wide-angle 1080p junction views where plates are 30–110 px wide: resized to `FRAME_WIDTH=960` and letterboxed into the 384 px model input they are 6–22 px and the ONNX detector finds none (survey `scripts/survey_anpr.py`, `media/anpr_survey.json`, `docs/anpr-camera-selection.md`). Recommended sandbox setting: `FRAME_WIDTH=1920 ANPR_DET_TILE=768 ANPR_DETECTOR=auto`. | Measured on the real sandbox 5 Sept |
| 2026-09-05 | anpr (sandbox survey) | §1.3 / §7.7 | **Static-box suppression.** New worker variable `ANPR_STATIC_BOX_WINDOW` (default `20`, `0` = off): per camera, `anpr.detector.StaticBoxFilter` drops a detector box when a box overlapping it (IoU ≥ 0.6) was present in ≥ 80 % of the last N frames (judged only after half the window). Burnt-in clocks / camera names and signboards are boxed with 0.9+ confidence and OCR'd at 0.9+ (`2314:16`, `75750 03008`, `DELIGHT P1 RLVD`); on the sandbox the live worker posted 808 such invalid-format reads from cam02 in 12 minutes. A parked vehicle is read during its first N frames and then muted. Applied after the edge filter and before the per-frame OCR budget in `pipeline.process_frame`. | Measured on the real sandbox 5 Sept |
| 2026-09-05 | anpr (real-feed stability) | §1.3 / §7.7 | **Decoder on real relay/organiser feeds.** RTSP sources are no longer `ffprobe`d before the dial (`ANPR_RTSP_PROBE=0`, default): the probe cost a second RTSP session per camera against a server that refuses bursts of dials and timed out at 15 s while the relay was still starting the source on demand (every live camera `reconnecting`, "ffprobe timed out after 15 s" restart storms). Codec/size/fps are parsed from ffmpeg's own `Input #0` / `Stream #0:0` stderr lines; `ANPR_RTSP_PROBE=1` restores the probe with `ANPR_PROBE_TIMEOUT_S` (45 s; files are always probed). ffmpeg `-timeout` (RTSP socket I/O) is `ANPR_RTSP_TIMEOUT_S` = 30 s (was 10). Frame-rate reduction for RTSP sources is time based (`select=isnan(prev_selected_t)+gte(t-prev_selected_t,1/fps)+lt(t,prev_selected_t)`, loop-jump safe, no source fps needed; the `-fps_mode passthrough` rule stands; file sources keep `select=not(mod(n,K))`); pre-index adds the same select with `1/PREINDEX_FPS`, so `PREINDEX_FPS=0.5` really halves the load. Watchdog: `ANPR_START_TIMEOUT_S` (90 s) for the first frame of a session, `ANPR_STALL_TIMEOUT_S` (60 s, was 20) afterwards; the 2→30 s backoff is reset only after a session that delivered frames for > 60 s. `anpr.decode.DialGate`: one decoder dials at a time, `ANPR_DIAL_SPACING_S` (3 s) apart, at start-up and on every reconnect; released when ffmpeg has parsed the input stream, on the first frame, after 45 s or when the session ends. `ANPR_DECODE_THREADS` (2) = ffmpeg `-threads` per decoder. `API_TIMEOUT_S` (30 s, was 15) is the worker's HTTP timeout; heartbeat `extra.dials` counts dials. | Every live camera `reconnecting` on the organiser feeds (docs/sandbox-progress.md, 19:20 IST finding) |
| 2026-09-05 | anpr (real-feed stability) | §1.3 / §7.2 / §7.7 | **Burnt-in OSD filter and invalid-read cap.** `anpr.detector.OsdMask` (per camera): a detector box centred in the top/bottom `ANPR_OSD_BAND` (0.08) of the frame is never a plate (every organiser camera burns its clock/caption there; cam02's clock produced 808 invalid reads in 12 min); static-text regions are learnt from `ANPR_OSD_WARMUP_S` (30 s) of samples (1/8-scale grayscale every 0.5 s: temporal std < 4 **and** Laplacian edge ≥ 24 in ≥ 80 % of the samples, grouped into components ≥ 24 px wide and ≤ 20 % of the frame high) and refreshed every 30 s on a rolling window; a box with ≥ 50 % of its area inside a region is dropped. Both run before `StaticBoxFilter` and before OCR. `ANPR_INVALID_READS_PER_MIN` (10, 0 = unlimited): per camera, at most N `is_valid_format=false` reads are posted per minute, the rest are counted and dropped (valid plates are never capped; invalid reads never raised alerts). Heartbeat `extra.suppression[camera_id]` = `{static, osd_band, osd_region, osd_regions_now, invalid_capped}`. | cam02 clock reads (docs/anpr-camera-selection.md §5) |
| 2026-09-05 | deploy (real-feed stability) | §1.3 | Compose passes the new worker variables (`ANPR_DET_TILE`, `ANPR_DET_TILE_OVERLAP`, `ANPR_STATIC_BOX_WINDOW`, `ANPR_OSD_BAND`, `ANPR_OSD_WARMUP_S`, `ANPR_INVALID_READS_PER_MIN`, `ANPR_MAX_BOXES`, `OCR_THREADS`, `ANPR_PROBE_TIMEOUT_S`, `ANPR_RTSP_PROBE`, `ANPR_RTSP_TIMEOUT_S`, `ANPR_START_TIMEOUT_S`, `ANPR_STALL_TIMEOUT_S`, `ANPR_DIAL_SPACING_S`, `ANPR_DECODE_THREADS`, `API_TIMEOUT_S`) through the `x-anpr-env` anchor (the tile/static-box variables added on 5 Sept were not wired before). Laptop real-feed profile in `deploy/.env`: `ANPR_MAX_CAMERAS=8 PREINDEX_MAX_CAMERAS=2 ANPR_FPS=2 PREINDEX_FPS=0.5 FRAME_WIDTH=1280 ANPR_DETECTOR=onnx ANPR_DET_TILE=768 ANPR_MIN_PLATE_W=28 OBJECT_DETECT=0 ANPR_LIVE_CPUS=8 ANPR_LIVE_MEMORY=4g ANPR_PREINDEX_CPUS=2 ANPR_PREINDEX_MEMORY=1536m`; GPU VM: `ANPR_MAX_CAMERAS=12 ANPR_FPS=5 FRAME_WIDTH=1920 ANPR_MIN_PLATE_W=40 OBJECT_DETECT=1 PREINDEX_MAX_CAMERAS=50 PREINDEX_FPS=1`. Root cause of the worker's `POST /internal/detections ReadTimeout`s measured on 5 Sept: not the API (handlers 8-50 ms, dir walk 0.2-0.4 s, fsync 6-17 ms) but Docker-VM memory exhaustion (7.96 GB total, 0.7 GB available, 2 GB swap fully used, 3.8 M pages swapped out) with the old live worker at 4.07 GB of its 4 GB cap plus survey/dry-run containers — the API process was paged out for up to 71 s; the caps above and the 30 s client timeout with 2→30 s retries are the fix. | docs/sandbox-progress.md |
| 2026-09-05 | anpr (real-feed accuracy) | §7.7 OCR / §1.3 | **Pluggable OCR backend** `ANPR_OCR=paddle\|fast_plate\|ensemble` (`anpr/ocr.py`, default `paddle`). `paddle` now runs on an *enhanced* crop (`enhance_crop`: 6 px replicate border, ×4 cubic upscale capped at 640 px, gamma correction when the crop is dark, polarity inversion for white-on-black plates, CLAHE) and recognises single-line crops (h/w < 0.45) with recognition only (`ANPR_OCR_PADDLE_MODE=auto`; `det` restores det+rec everywhere, two-line plates still go through det+rec). `fast_plate` = fast-plate-ocr 1.1.0 (MIT) with the `global_mobile_vit_v2_ocr` model (or `cct_s_v2_global` / `cct_xs_v2_global`, `ANPR_OCR_FAST_MODEL`), model + config hash-verified from `weights/HASHES.txt` like the detector. `ensemble` runs both: agreement on the normalised plate wins (+0.1 confidence), else a valid-format read beats an invalid one, else the higher confidence above `ANPR_OCR_ENSEMBLE_MIN_CONF` (0.45). The heartbeat carries `extra.ocr` (`backend`, `min_w`, `best_shot`, `detect_width`). Measured per backend on the labelled organiser crop bank in `docs/anpr-accuracy.md`. | Night footage: general OCR read one plate in the survey |
| 2026-09-05 | anpr (real-feed accuracy) | §7.7 Decode / Voting / §1.3 | **Native decode + downscaled detection + best-shot tracking.** `FRAME_WIDTH=0` decodes at the source resolution (no `scale` filter); `ANPR_DETECT_WIDTH` (0 = decoded width) runs the tiled detector and the OSD learner on a copy resized to that width and maps the boxes back, so OCR crops keep native pixels at the old detector cost. `ANPR_BEST_SHOT=1` (default): `anpr/tracks.py` `PlateTracker` associates boxes across frames per camera (greedy IoU ≥ 0.15, centre-distance fallback within 1.5 widths for 1-2 fps feeds), keeps the `ANPR_TRACK_KEEP` (3) best shots per track (width × sharpness, Laplacian variance on a 32 px-high copy) and OCRs them only when the track *settles* (best width unchanged for `ANPR_TRACK_STALL` = 3 frames), grows by ≥ 25 % since the last read, or *ends* (`ANPR_TRACK_GAP_S` = 2 s without a match); the 3 s char-wise vote then runs over those reads exactly as before. `ANPR_OCR_MIN_W` (60 px of the decoded frame) is the OCR floor; narrower boxes are tracked but never OCR'd. `ANPR_BEST_SHOT=0` keeps per-frame OCR. Sighting frames are resized to 960 px before the JPEG (§7.2) when the decode is wider. | 30→150 px approach read at its best, OCR budget |
| 2026-09-05 | anpr (real-feed accuracy) | §7.2 / §7.7 / §10 | **Detected-vehicle evidence.** Every ended plate track is a *detected vehicle*: the worker logs one INFO line (`camera N: vehicle tK w=… frames=… first..last plate=…`), counts it in the heartbeat `extra.vehicles[camera]` (`detected`, `ocr`, `read`, `open_tracks`, `evidence_files`, `ocr_calls`) and, when `ANPR_EVIDENCE_DIR` is set (compose: `/app/media/anpr_evidence`, bind-mounted from `./media/anpr_evidence` for `anpr-live` — the only write the worker makes), stores the best crop as `<dir>/<camera_id>/<YYYY-MM-DD>/<HHMMSS_mmm>_t<track>_w<px>.jpg` plus a JSON line in `vehicles.jsonl` (UTC first/last/best timestamps, bbox, width, frames, OCR result if any), capped at `ANPR_EVIDENCE_PER_HOUR` (200) per camera. The API is **not** involved: §7.2 accepts only `loop_reset` / `intrusion` events (`schemas/internal.py`, `ck_events_type`), so a `vehicle_detected` event type (crop + timestamp, shown in the output report) is a backend follow-up — until then the report's "detected vehicles" evidence is this store plus the worker log. | Portal rule: detected vehicles *or* plates with timestamps |
| 2026-09-05 | anpr (real-feed accuracy) | §7.7 OCR / §1.3 | **Letters-only OCR strings are never plates**: `pipeline._ocr_crop` drops a normalised OCR result without a digit before voting (counted in heartbeat `extra.suppression[camera].no_digit`). On the organiser crop bank 48 of the 71 widest detector boxes were signboards and burnt-in captions (`GUJARAT POLICE`, `Bhavani`, a Gujarati shop sign, `Junction`), which PaddleOCR reads at 0.9+ and which the static-box rule missed because the detector boxes them only intermittently; for the same reason `ANPR_STATIC_BOX_HITS` (default 0.8 = the old rule; `deploy/.env` sets 0.5 for the organiser feeds) now sets the share of the `ANPR_STATIC_BOX_WINDOW` frames a box must occupy to count as static. | Sign/caption false positives on night junction feeds |
| 2026-09-05 | anpr (real-feed accuracy, resume) | §7.5 / §7.7 / §10 | **Static-text tracks are not vehicles.** `anpr.tracks.track_kind(track)` classifies every ended plate track as `plate` (a valid-format read), `vehicle` (a plate box never read, or read as an invalid string) or `text` (every OCR pass returned letters only - a signboard / burnt-in caption the detector boxed; a registration always carries digits). `text` tracks are logged (`camera N: static text tK … (letters-only OCR xM, not a vehicle)`), counted in heartbeat `extra.vehicles[camera].text` and get **no** evidence file, so `extra.vehicles[camera].detected` and `media/anpr_evidence` contain vehicles only; each `vehicles.jsonl` record now carries `kind` (`plate` \| `vehicle`). A track flushed on a discontinuity / shutdown is read without a sighting frame (a crop was previously posted as the sighting's `frame`). Unit test `anpr/tests/test_tracks.py::test_track_kind_separates_plates_vehicles_and_static_text`. | Honest detected-vehicle counts on the organiser feeds (the crop bank showed 48 of the 71 widest detector boxes were signboards / captions) |
| 2026-09-05 | backend (relay policy) | §8.3 / §5.6 / §8.2 / §5.2 / §5.5 / §1.3 | **Relay-source policy ("open only what you process; pace your load").** `mediamtx_client.relay_policy(camera)` → **persistent** (`sourceOnDemand: false`) for every non-retired camera with `anpr_enabled=true` and for the own gate (`source='own'`), **on-demand** for everything else; both carry `sourceOnDemandCloseAfter: 600s` (was 60 s) so a wall viewer or a probe holds an idle sandbox copy for 10 min instead of re-dialling every minute; the `_h264` re-encode keeps `runOnDemandCloseAfter: 60s` (it reads the parent path, never upstream). MediaMTX therefore holds ONE upstream connection per processed camera and reconnects it itself (v1.20.1 retries a failed static source every fixed 5 s — `retryPause` in `internal/staticsources/handler.go`, no path-level option; the same rate an on-demand path with a waiting reader already retried at, so an outage is not made worse). `ensure_camera_paths` embeds the policy (import, `POST/PUT /cameras`, startup, poller self-heal); `ensure_relay_policy(camera)` PATCHes only `sourceOnDemand`/`sourceOnDemandCloseAfter` via `/v3/config/paths/patch` (no MediaMTX restart) and is called by the sandbox import for every non-retired camera (`sentinel_import.ensure_relay_policies`; import summary gains `relay_persistent`, `relay_policy_applied`) and by the health poller whenever a path's config drifts from the policy. Both compare against MediaMTX's own config echo first (`config_matches`, Go durations such as `10m0s` == `600s`) so an unchanged path is never PATCHed (MediaMTX restarts the source on every changed PATCH), and dial-inducing changes pass a shared `DialGate` (3 s apart). **Health poller**: a persistent camera is judged from relay state only (`ready`/`tracks`/`bytesReceived` delta; `tick_action` = `relay_only` when down) — never probed, no direct dial; an idle on-demand sandbox camera is probed at most `HEALTH_PROBE_PARALLEL`=2 at a time, 3 s apart (same gate), `HEALTH_PROBE_MAX`=2 per tick and once per `HEALTH_PROBE_MIN_INTERVAL_S`=900 s; between probes it keeps its status and fail count ("unknown, retry later": no health-log row) so only `HEALTH_OFFLINE_AFTER`=3 consecutive probe misses take it offline; a ready observation (wall viewer, worker) counts as a fresh probe, and the probe history is seeded from `camera_health_log` on the first tick after an API start so the spacing survives restarts. `HEALTH_PROBE_VIA` = `direct` (default: one short connection to the source, `sandbox.probe_timeout_s` cap) or `relay` (through `cam_<id>`, cap + 15 s — note the relay then keeps that copy for the 10-minute close-after, ≈ 15 idle streams held on average at a 15-min interval, which is why `direct` is the default); non-sandbox external sources keep the direct probe. `/health/summary.mediamtx` gains `persistent` / `persistent_ready`. `deploy/.env`, `.env.example` and compose defaults carry the new values (`HEALTH_PROBE_MAX=2`, `HEALTH_PROBE_PARALLEL=2`, `HEALTH_PROBE_MIN_INTERVAL_S=900`, `HEALTH_PROBE_VIA=direct`); `deploy/mediamtx.yml` `pathDefaults.sourceOnDemandCloseAfter: 600s`. Tests: `tests/test_relay_policy.py` (DB-free). | 21:32 IST 5 Sept: 245 "connection refused" + 57 "timed out" from the organiser server in 15 min, 8 live cameras restarting 2-5× each in 30 min at 1.7 fps total — all self-inflicted dial churn (on-demand relay paths re-dialling for the worker/wall/probes, poller dialling the sandbox directly on top) |
| 2026-09-05 | backend (government-feed evidence) | §12.2 / §5.15 | **Watchlist seed rev 3** (`app.seed.WATCHLIST_SEED_REV = 3`): `backend/seeds/watchlist_seed.csv` keeps the six synthetic anchor rows of §12.2 unchanged and replaces the fourteen filler plates (old rows 7-20) with plates **actually read on the organiser cameras** on 5 Sept 2026 (cam07: `GJ32AG2319`, `GJ14AA3978`, `GJ32K0225`, `GJ10OG9093`, `GJ3B7160`, `GJ10F0032`, `GJ32AA4959`, `GJ32AG5511` from `/api/detections`, `GJ32X9785` from the readability survey, `GJ36AJ2890` / `GJ32D3783` from the hand-labelled crop bank - every note names camera, IST time, confidence and the by-eye reading); the three person rows stay. On an existing database the rev-3 upgrade **deactivates** (never deletes) the fourteen fillers (`RETIRED_FILLER_PLATES`, note appended) so alert history and hit counts survive, then adds the missing rows; `seed.watchlist_rev` = 3. `tests/test_seeds.py` asserts 20 rows / 11 government-feed plates. |
| 2026-09-05 | backend (government-feed evidence) | §5.15 | `GET /reports/detections` and `GET /reports/quality` accept **`source`** (`sandbox` \| `csv` \| `api` \| `manual` \| `own`, a `cameras.source` value; unknown → `422 {"field":"source"}`): reads, sightings, object counts, alerts and the quality section are restricted to cameras of that source, so the government-feed report can be produced without the own gate or mock rows; the filter is echoed in the CSV trailer / PDF filters line, the report-history `filters` and the quality JSON `window.source`; the quality PDF header prints the scope. `docs/tools/build-evidence-report.mjs` (Node, running stack) writes the companion **government-feed evidence report** (`docs/export/evidence_<from>_<to>_IST.pdf/.csv/.json`): every OCR read of the window from `/api/detections` next to every **detected vehicle** of the worker's evidence store (`media/anpr_evidence/<camera>/<date>/vehicles.jsonl`, best-shot crop, IST time, camera id, SHA-256), static-text tracks excluded, plus the watchlist alerts on organiser cameras. |
